...
impl<'a> SplitDoc<'a> {
    pub(crate) fn split(buf: &'a [u8]) -> Result<SplitDoc> {
        let (&compress_raw, buf) = buf.split_first().ok_or_else(|| Error::LengthTooShort { step: "get compress type", actual: 0, expected: 1 })?;
        let (hash_len, buf) = buf.split_first().ok_or_else(|| Error::LengthTooShort { step: "get hash length", actual: 0, expected: 1 })?;
        let hash_len = *hash_len as usize;
        if hash_len > 127 {
            return Err(Error::BadHeader(format!("Hash length must be 0-127, marked as {}", hash_len)));
        }
        if buf.len() < hash_len + 3 {
            return Err(Error::LengthTooShort { step: "get hash then data length", actual: buf.len(), expected: hash_len+3 });
        }
...
impl NewDocument {
    ...
    pub fn new<S: Serialize>(data: S, schema: Option<&Hash>) -> Result<Self> {
        ...
        if buf.len() > MAX_DOC_SIZE {
            return Err(Error::LengthTooLong { max: MAX_DOC_SIZE, actual: buf.len() });
        }
    ...
    pub fn sign(mut self, key: &IdentityKey) -> Result<Self> {
        ...
        if new_len > MAX_DOC_SIZE {
            return Err(Error::LengthTooLong { max: MAX_DOC_SIZE, actual: self.buf.len() });
        }
    ...
    pub(crate) fn data(&self) -> &[u8] {
        SplitDoc::split(&self.buf).unwrap().data
    }
...
*/
    ...
    fn sign_roundtrip() {
        let key = IdentityKey::new_temp(&mut rand::rngs::OsRng);
        let new_doc = NewDocument::new(&1u8, None)
            .unwrap()
            .sign(&key)
            .unwrap();
        assert_eq!(new_doc.data(), &[1u8]);
